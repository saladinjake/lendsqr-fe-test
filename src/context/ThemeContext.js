import React, { createElement, Component, useContext, useState, useEffect } from "react";
...
function genComponentStyle(tag) {
  return function(strings, ...vals) {
    return class extends Component {
      ...
      componentWillMount() {
        if (this.context) this.props = { ...this.props, theme: this.context };
        this.style = computeStyle(this.props, strings, vals);
      }
      componentWillUpdate(props) {
        if (this.context) props = { ...props, theme: this.context };
      ...
      render() {
        ...
        return createElement(tag, { style: this.style, ...props }, [
          ...props.children
        ]);
      }
    };
...
const cssStyled = {};
tags.forEach(tag => {
  cssStyled[tag] = genComponentStyle(tag);
});
